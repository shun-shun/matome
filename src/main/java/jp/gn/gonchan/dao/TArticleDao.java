...
public interface TArticleDao {
    ...
    List<MatomeDto> selectListRandamtest();
    ...
    List<MatomeDto> selectListNewArrivals(@Bind("startTimeStamp") Timestamp startTimeStamp,
    ...
    @SqlQuery("SELECT T_ARTICLES.destinationId, articleId, title,link,DiestinationName,publishedDate "
            ...
            + "LIMIT :displayCount OFFSET :offset;")
    List<MatomeDto> selectListNewArrivalsByBlog(@Bind("destinationId") int destinationId,
    ...
    @SingleValueResult
    Optional<MatomeDto> selectArticleById(@Bind("destinationId") int destinationId, @Bind("articleId") int articleId);


    @SqlQuery("SELECT count(*) FROM T_ARTICLES WHERE publishedDate BETWEEN :startTimeStamp AND :endTimeStamp")
    int selectTodayArticles(@Bind("startTimeStamp") Timestamp startTimeStamp,
            @Bind("endTimeStamp") Timestamp endTimeStamp);

    @SqlQuery("SELECT count(*) FROM T_ARTICLES WHERE destinationId = :destinationId")
    int selectTotalByDestination(@Bind("destinationId") int destinationId);
}
