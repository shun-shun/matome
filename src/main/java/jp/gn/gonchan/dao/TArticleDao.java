...
public abstract class TArticleDao {
    ...
    public abstract void insertArticle(@BindBean Articles articles);

    @SqlQuery("SELECT DESTINATIONID FROM T_ARTICLES WHERE title = :title")
    @SingleValueResult
    public abstract Optional<Integer> selectArticleByTitle(@Bind("title") String title);


    @SqlQuery("SELECT * FROM T_ARTICLES WHERE destinationId = :destinationId ORDER BY ASC LIMIT 10")
    public abstract List<Articles> selectListByIdLim10(@Bind("id") int destinationId);

    @SqlQuery("SELECT * FROM T_ARTICLES ORDER BY RAND() LIMIT 10")
    public abstract List<Articles> selectListRandam();
    ...
    public abstract List<MatomeDto> selectListRandamtest();
    ...
    public abstract List<MatomeDto> selectListNewArrivals(@Bind("startTimeStamp") Timestamp startTimeStamp,
    ...
    @SqlQuery("SELECT T_ARTICLES.destinationId, articleId, title,link,DiestinationName,publishedDate "
            ...
            + "LIMIT :displayCount OFFSET :offset;")
    public abstract List<MatomeDto> selectListNewArrivalsByBlog(@Bind("destinationId") int destinationId,
    ...
    @SingleValueResult
    public abstract Optional<MatomeDto> selectArticleById(@Bind("destinationId") int destinationId,
    ...
    public abstract int selectTodayArticles(@Bind("startTimeStamp") Timestamp startTimeStamp,
            @Bind("endTimeStamp") Timestamp endTimeStamp);

    @SqlQuery("SELECT count(*) FROM T_ARTICLES WHERE destinationId = :destinationId")
    public abstract int selectTotalByDestination(@Bind("destinationId") int destinationId);
}
